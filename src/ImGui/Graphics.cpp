	...
	bool Texture::Load(bool a_resizeToScreenRes)
	{
		...
		if (SUCCEEDED(hr)) {
			if (auto renderer = RE::BSGraphics::Renderer::GetSingleton()) {
				...
				hr = DirectX::CreateTexture((ID3D11Device*)renderer->GetRendererDataSingleton()->forwarder, image->GetImages(), 1, image->GetMetadata(), &pTexture);
				...
				if (SUCCEEDED(hr)) {
					...
					hr = ((ID3D11Device*)renderer->GetRendererDataSingleton()->forwarder)->CreateShaderResourceView(pTexture.Get(), &srvDesc, &srView);
					result = SUCCEEDED(hr);
				}
