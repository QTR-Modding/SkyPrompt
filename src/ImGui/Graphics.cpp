        ...
        if (SUCCEEDED(hr)) {
            if (auto renderer = RE::BSGraphics::Renderer::GetSingleton()) {
                if (a_resizeToScreenRes) {
                    ...
                    if (screenSize.height != image->GetMetadata().height && screenSize.width != image->GetMetadata().
                        width) {
                        DirectX::ScratchImage tmpImage;
                        DirectX::Resize(*image->GetImage(0, 0, 0), screenSize.width, screenSize.height,
                                        DirectX::TEX_FILTER_CUBIC, tmpImage);
                ...
                hr = DirectX::CreateTexture((ID3D11Device*)renderer->GetRendererDataSingleton()->forwarder,
                                            image->GetImages(), 1, image->GetMetadata(), &pTexture);
                ...
                if (SUCCEEDED(hr)) {
                    ...
                    hr = ((ID3D11Device*)renderer->GetRendererDataSingleton()->forwarder)->CreateShaderResourceView(
                        pTexture.Get(), &srvDesc, &srView);
                    result = SUCCEEDED(hr);
                }
